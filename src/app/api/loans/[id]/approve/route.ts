...
export async function POST(
  ...
  try {
    ...
    await eventManager.createEvent(
      EventHelpers.loanApproved(loanId, 'SYSTEM') // In real app, would be operator ID
    );
    ...
    try {
      if (mockMode || process.env.NODE_ENV === 'development') {
        ...
        // Real on-chain disbursement
        const trustLendContract = createTrustLendContract();
        txHash = await trustLendContract.disburseLoan(loanId, BigInt(loan.valorTotal));
      }
      ...
      if (txHash) {
        ...
        await eventManager.createEvent(
          EventHelpers.loanDisbursed(loanId, txHash)
        );
      }
    } catch (error) {
