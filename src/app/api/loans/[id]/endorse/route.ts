...
export async function POST(
  ...
  try {
    ...
    if (!supporter) {
      return NextResponse.json(createApiError("Apoiador não encontrado"), {
        status: 404,
      });
    }

    if (supporter.tipo !== "APOIADOR") {
      return NextResponse.json(createApiError("Usuário não é um apoiador"), {
        status: 400,
      });
    }

    if (supporter.status !== "ATIVO") {
      return NextResponse.json(createApiError("Apoiador não está ativo"), {
        status: 400,
      });
    }
    ...
    if (loan.tomadorId === validatedData.apoiadorId) {
      return NextResponse.json(createApiError("Auto-apoio não é permitido"), {
        status: 400,
      });
    }
    ...
    if (validatedData.valorStake > maxStakePerEndorser) {
      return NextResponse.json(
        createApiError(
          `Valor máximo por apoiador: ${(
            maxStakePerEndorser / 1_000_000
          ).toFixed(2)} USDC`
        ),
        { status: 400 }
      );
    ...
    const supporterLoans = await repository.getLoansForUser(
      validatedData.apoiadorId,
      "APOIADOR"
    );
    const activeLoans = supporterLoans.filter((l) =>
      ["ATIVO", "APROVADO"].includes(l.estado)
    );
    ...
    const totalStaked = allEndorsements.reduce(
      (sum, e) => sum + e.valorStake,
      0
    );
    ...
    const fraudAlerts = FraudDetector.runComprehensiveCheck(loanInfo, users as any);
    ...
    const concentrationAlert = fraudAlerts.find(
      (a) => a.tipo === "CONCENTRACAO"
    );
    ...
    await eventManager.createEvent(
      EventHelpers.loanEndorsed(
        loanId,
        endorsement.id,
        validatedData.apoiadorId,
        validatedData.valorStake
      )
    );
